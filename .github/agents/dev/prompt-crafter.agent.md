...
```text
1. Search for related code:    search for similar functionality
2. Check existing patterns:    search for similar file structures
...
{Clarifying questions presented directly in the conversation}
```
...
> - `AskUserQuestion` — Claude Code's structured question UI; no direct Copilot equivalent.
> - `Task` — Claude Code's background task runner; no direct Copilot equivalent.
>
> For interactive clarification, ask questions directly in the conversation response.
