...
Glob(.github/           agents:                 main.py → @function-developer
...
|--------|---------|-------------|
| **pydantic** | Data validation, LLM output parsing | `.github/kb/pydantic/` |
| **gcp** | Cloud Run, Pub/Sub, GCS, BigQuery | `.github/kb/gcp/` |
| **gemini** | Document extraction, vision tasks | `.github/kb/gemini/` |
| **langfuse** | LLM observability | `.github/kb/langfuse/` |
| **terraform** | Infrastructure as Code | `.github/kb/terraform/` |
| **terragrunt** | Multi-environment orchestration | `.github/kb/terragrunt/` |
| **crewai** | Multi-agent orchestration | `.github/kb/crewai/` |
| **openrouter** | LLM fallback provider | `.github/kb/openrouter/` |
...
/workflow_brainstorm "Build an invoice extraction system"

# Phase 1: Define requirements with Technical Context
/workflow_define .github/sdd/features/BRAINSTORM_INVOICE_EXTRACTION.md
...
/workflow_build .github/sdd/features/DESIGN_INVOICE_EXTRACTION.md

# Phase 4: Archive
/workflow_ship .github/sdd/features/DEFINE_INVOICE_EXTRACTION.md
```
...
/workflow_define "Build a REST API endpoint for user authentication"

# Continue: /workflow_design → /workflow_build → /workflow_ship
```
...
```bash
# Update any phase
/workflow_iterate DEFINE_AUTH.md "Add OAuth support requirement"
/workflow_iterate DESIGN_AUTH.md "Change to use JWT tokens"
```
...
"I'll just write the code"    vs    "Let me /workflow_define first"
        │                                   │
        ▼                                   ▼
...
touch .github/agents/data-engineering/dbt-specialist.agent.md
```
...
1. **The agent is automatically discoverable** - Design phase will find it via `Glob(.github/agents/**/*.md)`
...
```bash
mkdir -p .github/kb/dbt
touch .github/kb/dbt/{index.md,quick-reference.md}
mkdir -p .github/kb/dbt/{concepts,patterns}
```
...
domains:
  dbt:
    ...
    entry_point: ".github/kb/dbt/index.md"
```
...
| Knowledge Base | `.github/kb/` |
| Prompts | `.github/prompts/workflow_*` |
