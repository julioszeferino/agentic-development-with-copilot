...
| Option | Effect |
|--------|--------|
| (no argument) | Review most recently modified deck in `presentation/` |
| `d1-ingest` | Review specific deck |
| `d2-context.html` | Review by filename |
| `--fix` | Review + auto-fix (invokes fixer) |
| `d1-ingest --fix` | Review specific deck + auto-fix |

---

## Overview

This prompt invokes the **aide-slide-reviewer** agent for independent, read-only validation of any slide deck against the AIDE quality KB. Optionally pairs with **aide-slide-fixer** for automated corrections.
...
- .github/kb/aide-slides/semana-palette.md (if deck is d1-d4)
...
```bash
# The build-slides prompt includes review automatically,
...
- `build-slides` prompt — Full pipeline: plan + build + review + fix
