...
| Option | Effect |
|--------|--------|
| `<DOMAIN>` | Create new KB domain from scratch |
| `--audit` | Audit all existing KB domains for health, completeness, and template compliance |

**Examples**: `/create-kb redis`, `/create-kb pandas`, `/create-kb authentication`

## What Happens

1. **Validates prerequisites** — checks `.github/kb/_templates/` exist
...
- Scan all directories under `.github/kb/`
...
Created:
  .github/kb/<domain>/index.md
  .github/kb/<domain>/quick-reference.md
  .github/kb/<domain>/concepts/<concept-1>.md
  .github/kb/<domain>/concepts/<concept-2>.md
  .github/kb/<domain>/patterns/<pattern-1>.md
```
...
| Templates missing | Error with: "Run from repo root. Check `.github/kb/_templates/` exists" |
...
- **Example KB**: `.github/kb/pydantic/` (well-structured reference)
